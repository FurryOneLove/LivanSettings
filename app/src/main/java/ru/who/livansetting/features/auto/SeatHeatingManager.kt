...
class SeatHeatingManager(private val context: Context) {
    ...
    fun toggleDriverSeatHeat() {
        ...
        val currentLevel = carService.getFunctionValue(FUNC_ID_DRIVER_SEAT_HEAT) ?: HEAT_LEVEL_OFF
    ...
    fun togglePassengerSeatHeat() {
        val carService = getCarService() ?: return
        val currentLevel = carService.getFunctionValue(FUNC_ID_PASSENGER_SEAT_HEAT) ?: HEAT_LEVEL_OFF
        val nextLevel = (currentLevel + 1) % 4
    ...
    fun getDriverSeatHeatLevel() = getCarService()?.getFunctionValue(FUNC_ID_DRIVER_SEAT_HEAT)
    fun getPassengerSeatHeatLevel() = getCarService()?.getFunctionValue(FUNC_ID_PASSENGER_SEAT_HEAT)
}
