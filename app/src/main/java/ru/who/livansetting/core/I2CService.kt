...
class I2CService(private val context: Context) {
    ...
    private fun processKeyData(data: IntArray) {
        ...
        if (!flavor601 && data.size > IICKeyCodes.IIC_DATA_INDEX_VOLUME_UP) {
            if ((data[IICKeyCodes.IIC_DATA_INDEX_VOLUME_UP] and IICKeyCodes.IIC_PRESS_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_VOLUME_UP, false)
            if ((data[IICKeyCodes.IIC_DATA_INDEX_VOLUME_UP] and IICKeyCodes.IIC_RELEASE_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_VOLUME_UP, true)
        }
        
        // Mute
        if (data.size > IICKeyCodes.IIC_DATA_INDEX_MUTE) {
            if ((data[IICKeyCodes.IIC_DATA_INDEX_MUTE] and IICKeyCodes.IIC_PRESS_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_MUTE, false)
            if ((data[IICKeyCodes.IIC_DATA_INDEX_MUTE] and IICKeyCodes.IIC_RELEASE_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_MUTE, true)
        }
        
        // Volume Down
        if (!flavor601 && data.size > IICKeyCodes.IIC_DATA_INDEX_VOLUME_DOWN) {
            if ((data[IICKeyCodes.IIC_DATA_INDEX_VOLUME_DOWN] and IICKeyCodes.IIC_PRESS_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_VOLUME_DOWN, false)
            if ((data[IICKeyCodes.IIC_DATA_INDEX_VOLUME_DOWN] and IICKeyCodes.IIC_RELEASE_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_VOLUME_DOWN, true)
        }
        
        // Power
        if (data.size > IICKeyCodes.IIC_DATA_INDEX_POWER) {
            if ((data[IICKeyCodes.IIC_DATA_INDEX_POWER] and IICKeyCodes.IIC_PRESS_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_POWER, false)
            if ((data[IICKeyCodes.IIC_DATA_INDEX_POWER] and IICKeyCodes.IIC_RELEASE_MASK) != 0) handleKey(IICKeyCodes.KEY_CODE_IIC_POWER, true)
        }
    }
